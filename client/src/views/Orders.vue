    ...
    <div v-else>
      ...
      <div v-if="restockingOrders.length > 0" class="card restocking-orders-section">
        <div class="card-header">
          ...
          <span class="badge info">Internal Orders</span>
        </div>
          ...
          <table class="restocking-table">
            ...
            <tbody>
              <tr v-for="order in restockingOrders" :key="order.id">
                  ...
                  <details class="items-details">
                    ...
                    <div class="items-dropdown">
                      <div v-for="(item, idx) in order.items" :key="idx" class="item-entry">
                        ...
                        <span class="item-meta">{{ item.sku }} — Qty: {{ item.quantity }} @ {{ currencySymbol }}{{ item.unit_price }}</span>
                      </div>
                    </div>
                  </details>
                </td>
                ...
                <td class="col-lead-time">{{ calculateLeadTime(order.order_date, order.expected_delivery) }} days</td>
...
export default {
  ...
  setup() {
    ...
    const restockingOrders = computed(() =>
      orders.value
        .filter(o => o.category === 'Restocking')
        .sort((a, b) => new Date(b.order_date) - new Date(a.order_date))
    )

    const regularOrders = computed(() =>
      orders.value.filter(o => o.category !== 'Restocking')
    )
...
.restocking-table .col-items {
  width: 200px;
}

.restocking-table .col-status {
  width: 110px;
}

.restocking-table .col-date {
  width: 130px;
}

.restocking-table .col-value {
  width: 120px;
}
</style>
